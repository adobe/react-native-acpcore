...
import { ACPExtensionEvent } from "./models/ACPExtensionEvent";
import { ACPMobileLogLevel } from "./models/ACPMobileLogLevel";
import { ACPMobilePrivacyStatus } from "./models/ACPMobilePrivacyStatus";

interface IACPCore {
  extensionVersion: () => Promise<string>;
  configureWithAppId: (appId?: string) => void;
  updateConfiguration: (configMap?: { [key: string]: any }) => void;
  setLogLevel: (mode: ACPMobileLogLevel) => void;
  getLogLevel: () => Promise<ACPMobileLogLevel>;
  log: (logLevel: ACPMobileLogLevel, tag: string, message: string) => void;
  setPrivacyStatus: (privacyStatus: ACPMobilePrivacyStatus) => void;
  getPrivacyStatus: () => Promise<ACPMobilePrivacyStatus>;
  getSdkIdentities: () => Promise<string | null>;
  dispatchEvent: (event: ACPExtensionEvent) => Promise<boolean>;
  dispatchEventWithResponseCallback: (
    event: ACPExtensionEvent
  ) => Promise<ACPExtensionEvent>;
  dispatchResponseEvent: (
    responseEvent: ACPExtensionEvent,
    requestEvent: ACPExtensionEvent
  ) => Promise<boolean>;
  trackAction: (action?: string, contextData?: { [key: string]: string }) => void;
  trackState: (state?: string, contextData?: { [key: string]: string }) => void;
  setAdvertisingIdentifier: (advertisingIdentifier?: string) => void;
  setPushIdentifier: (pushIdentifier?: string) => void;
  collectPii: (data: { [key: string]: string }) => void;
...
const RCTACPCore: IACPCore = NativeModules.ACPCore;

const ACPCore: IACPCore = {
  /**
   ...
   */
  updateConfiguration(configMap?: { [key: string]: any }) {
   ...
   */
  trackAction(action?: string, contextData?: { [key: string]: string }) {
   ...
   * @param  {{ string: string }?} contextData containing context data to attach on this hit
   */
  trackState(state?: string, contextData?: { [key: string]: string }) {
   ...
   */
  collectPii(data: { [key: string]: string }) {
