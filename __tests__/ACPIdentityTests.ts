...
import ACPIdentity from "../ts/ACPIdentity";
import { ACPMobileVisitorAuthenticationState } from "../ts/models/ACPMobileVisitorAuthenticationState";
...
describe("ACPIdentity", () => {
  ...
  test("syncIdentifiersWithAuthState is called with correct parameters", async () => {
    const spy = jest.spyOn(
      NativeModules.ACPIdentity,
      "syncIdentifiersWithAuthState"
    );
    let identifiers = { testKey: "testValue" };
    let authState =
      ACPMobileVisitorAuthenticationState.ACP_VISITOR_AUTH_STATE_LOGGED_OUT;
  ...
  test("syncIdentifier is called with correct parameters", async () => {
    ...
    let authState =
      ACPMobileVisitorAuthenticationState.ACP_VISITOR_AUTH_STATE_AUTHENTICATED;
